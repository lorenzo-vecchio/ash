    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Value::Variant(name, fields) => {
                if fields.is_empty() { write!(f, "{name}") }
                else {
...
impl Default for Env {
    fn default() -> Self { Env { globals: Rc::new(RefCell::new(HashMap::new())), locals: vec![] } }
}

impl Env {
    pub fn new() -> Self {
        let mut e = Env { globals: Rc::new(RefCell::new(HashMap::new())), locals: vec![] };
        e.register_stdlib();
        e
    }
    ...
    pub fn child(&self) -> Self {
        Env { globals: Rc::clone(&self.globals), locals: vec![] }
    }

    pub fn push(&mut self) { self.locals.push(HashMap::new()); }
    pub fn pop(&mut self)  { self.locals.pop(); }
    ...
    pub fn set(&mut self, name: &str, val: Value) {
        for frame in self.locals.iter_mut().rev() {
            if frame.contains_key(name) { frame.insert(name.to_string(), val); return; }
        }
    ...
    fn register_stdlib(&mut self) {
        // println
        self.define("println", Value::Fn(FnValue {
        ...
        // print
        self.define("print", Value::Fn(FnValue {
        ...
        // int()
        self.define("int", Value::Fn(FnValue {
            name: Some("int".into()),
            params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                match args.first() {
                    ...
                    Some(Value::Str(s))   => s.parse::<i64>().map(Value::Int).map_err(|_| InterpError::runtime(format!("cannot convert '{s}' to int"))),
        ...
        // float()
        self.define("float", Value::Fn(FnValue {
            ...
            body: FnBody::Native(std::sync::Arc::new(|args| {
                match args.first() {
                    ...
                    Some(Value::Str(s))   => s.parse::<f64>().map(Value::Float).map_err(|_| InterpError::runtime(format!("cannot convert '{s}' to float"))),
        ...
        // str()
        self.define("str", Value::Fn(FnValue {
            name: Some("str".into()),
            params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
        ...
        // bool()
        self.define("bool", Value::Fn(FnValue {
            name: Some("bool".into()),
            params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                match args.first() {
        ...
        // abs, min, max
        self.define("abs", Value::Fn(FnValue {
            name: Some("abs".into()), params: vec![],
        ...
        self.define("min", Value::Fn(FnValue {
            name: Some("min".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                if args.len() != 2 { return Err(InterpError::runtime("min() requires 2 args")); }
                match (&args[0], &args[1]) {
        ...
        self.define("max", Value::Fn(FnValue {
            name: Some("max".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                if args.len() != 2 { return Err(InterpError::runtime("max() requires 2 args")); }
                match (&args[0], &args[1]) {
        ...
        // fmt
        self.define("fmt", Value::Fn(FnValue {
            name: Some("fmt".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                if args.is_empty() { return Err(InterpError::runtime("fmt() requires at least 1 arg")); }
        ...
        self.define("filter", Value::Fn(FnValue {
            name: Some("filter".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|_args| {
                // Will be called through the interpreter's call mechanism
                Err(InterpError::runtime("filter() must be called through interpreter"))
            })),
            closure: Env::default(),
        }));
        // map(list, fn)
        self.define("map", Value::Fn(FnValue {
            name: Some("map".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|_args| {
                Err(InterpError::runtime("map() must be called through interpreter"))
            })),
            closure: Env::default(),
        }));

        // -- reduce(list, fn, init) ------------------------------------------
        self.define("reduce", Value::Fn(FnValue {
            name: Some("reduce".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|_args| {
                Err(InterpError::runtime("reduce() must be called through interpreter"))
            })),
            closure: Env::default(),
        }));

        // -- zip(a b) --------------------------------------------------------
        self.define("zip", Value::Fn(FnValue {
            name: Some("zip".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                if args.len() < 2 { return Err(InterpError::runtime("zip() requires 2 lists")); }
                match (&args[0], &args[1]) {
                    (Value::List(a), Value::List(b)) => {
                        Ok(Value::List(a.iter().zip(b.iter()).map(|(x,y)| Value::Tuple(vec![x.clone(),y.clone()])).collect()))
                    }
        ...
        // -- flat(list) ------------------------------------------------------
        self.define("flat", Value::Fn(FnValue {
            name: Some("flat".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                match args.first() {
        ...
        // -- any(list, fn) ---------------------------------------------------
        self.define("any", Value::Fn(FnValue {
            name: Some("any".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|_args| {
                Err(InterpError::runtime("any() must be called through interpreter"))
            })),
            closure: Env::default(),
        }));

        // -- all(list, fn) ---------------------------------------------------
        self.define("all", Value::Fn(FnValue {
            name: Some("all".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|_args| {
                Err(InterpError::runtime("all() must be called through interpreter"))
            })),
            closure: Env::default(),
        }));

        // -- clamp(x lo hi) --------------------------------------------------
        self.define("clamp", Value::Fn(FnValue {
            name: Some("clamp".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
                if args.len() < 3 { return Err(InterpError::runtime("clamp() requires 3 args")); }
        ...
        macro_rules! math_fn1 {
            ($name:expr, $op:expr) => {
                self.define($name, Value::Fn(FnValue {
                    name: Some($name.into()), params: vec![],
                    body: FnBody::Native(std::sync::Arc::new(|args| {
                        match args.first() {
        ...
        self.define("math.floor",  Value::Fn(FnValue { name: Some("math.floor".into()),  params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.floor())), Some(Value::Int(n))=>Ok(Value::Float((*n as f64).floor())), _=>Err(InterpError::runtime("math.floor requires number")) } })), closure: Env::default() }));
        self.define("math.ceil",   Value::Fn(FnValue { name: Some("math.ceil".into()),   params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.ceil())),  Some(Value::Int(n))=>Ok(Value::Float((*n as f64).ceil())),  _=>Err(InterpError::runtime("math.ceil requires number"))  } })), closure: Env::default() }));
        self.define("math.round",  Value::Fn(FnValue { name: Some("math.round".into()),  params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.round())), Some(Value::Int(n))=>Ok(Value::Float((*n as f64).round())), _=>Err(InterpError::runtime("math.round requires number")) } })), closure: Env::default() }));
        self.define("math.sqrt",   Value::Fn(FnValue { name: Some("math.sqrt".into()),   params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.sqrt())),  Some(Value::Int(n))=>Ok(Value::Float((*n as f64).sqrt())),  _=>Err(InterpError::runtime("math.sqrt requires number"))  } })), closure: Env::default() }));
        self.define("math.sin",    Value::Fn(FnValue { name: Some("math.sin".into()),    params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.sin())),   Some(Value::Int(n))=>Ok(Value::Float((*n as f64).sin())),   _=>Err(InterpError::runtime("math.sin requires number"))   } })), closure: Env::default() }));
        self.define("math.cos",    Value::Fn(FnValue { name: Some("math.cos".into()),    params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.cos())),   Some(Value::Int(n))=>Ok(Value::Float((*n as f64).cos())),   _=>Err(InterpError::runtime("math.cos requires number"))   } })), closure: Env::default() }));
        self.define("math.tan",    Value::Fn(FnValue { name: Some("math.tan".into()),    params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.tan())),   Some(Value::Int(n))=>Ok(Value::Float((*n as f64).tan())),   _=>Err(InterpError::runtime("math.tan requires number"))   } })), closure: Env::default() }));
        self.define("math.log",    Value::Fn(FnValue { name: Some("math.log".into()),    params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.ln())),    Some(Value::Int(n))=>Ok(Value::Float((*n as f64).ln())),    _=>Err(InterpError::runtime("math.log requires number"))   } })), closure: Env::default() }));
        self.define("math.log2",   Value::Fn(FnValue { name: Some("math.log2".into()),   params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.log2())),  Some(Value::Int(n))=>Ok(Value::Float((*n as f64).log2())),  _=>Err(InterpError::runtime("math.log2 requires number"))  } })), closure: Env::default() }));
        self.define("math.log10",  Value::Fn(FnValue { name: Some("math.log10".into()),  params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { match args.first() { Some(Value::Float(f))=>Ok(Value::Float(f.log10())), Some(Value::Int(n))=>Ok(Value::Float((*n as f64).log10())), _=>Err(InterpError::runtime("math.log10 requires number")) } })), closure: Env::default() }));
        self.define("math.pi",     Value::Float(std::f64::consts::PI));
        self.define("math.e",      Value::Float(std::f64::consts::E));
        self.define("math.pow", Value::Fn(FnValue { name: Some("math.pow".into()), params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { if args.len()<2{return Err(InterpError::runtime("math.pow requires 2 args"));} match (&args[0],&args[1]) { (Value::Float(x),Value::Float(e))=>Ok(Value::Float(x.powf(*e))), (Value::Int(x),Value::Int(e))=>Ok(Value::Float((*x as f64).powf(*e as f64))), _=>Err(InterpError::runtime("math.pow requires numbers")) } })), closure: Env::default() }));

        // -- env.* -----------------------------------------------------------
        self.define("env.get",     Value::Fn(FnValue { name: Some("env.get".into()),     params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { let key=match args.first(){Some(Value::Str(s))=>s.clone(),_=>return Err(InterpError::runtime("env.get requires string key"))}; Ok(std::env::var(&key).ok().map(|v|Value::Option(Some(Box::new(Value::Str(v))))).unwrap_or(Value::Option(None))) })), closure: Env::default() }));
        self.define("env.require", Value::Fn(FnValue { name: Some("env.require".into()), params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { let key=match args.first(){Some(Value::Str(s))=>s.clone(),_=>return Err(InterpError::runtime("env.require requires string key"))}; std::env::var(&key).map(Value::Str).map_err(|_|InterpError::runtime(format!("required env var '{key}' not set"))) })), closure: Env::default() }));

        // -- file.* ----------------------------------------------------------
        self.define("file.read",   Value::Fn(FnValue { name: Some("file.read".into()),   params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { let path=match args.first(){Some(Value::Str(s))=>s.clone(),_=>return Err(InterpError::runtime("file.read requires path"))}; Ok(std::fs::read_to_string(&path).ok().map(|s|Value::Option(Some(Box::new(Value::Str(s))))).unwrap_or(Value::Option(None))) })), closure: Env::default() }));
        self.define("file.write",  Value::Fn(FnValue { name: Some("file.write".into()),  params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { if args.len()<2{return Err(InterpError::runtime("file.write requires path and data"));} let (path,data)=match(&args[0],&args[1]){(Value::Str(p),Value::Str(d))=>(p.clone(),d.clone()),_=>return Err(InterpError::runtime("file.write requires strings"))}; std::fs::write(&path,&data).map(|_|Value::Unit).map_err(|e|InterpError::runtime(e.to_string())) })), closure: Env::default() }));
        self.define("file.exists", Value::Fn(FnValue { name: Some("file.exists".into()), params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { let path=match args.first(){Some(Value::Str(s))=>s.clone(),_=>return Err(InterpError::runtime("file.exists requires path"))}; Ok(Value::Bool(std::path::Path::new(&path).exists())) })), closure: Env::default() }));
        self.define("file.rm",     Value::Fn(FnValue { name: Some("file.rm".into()),     params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { let path=match args.first(){Some(Value::Str(s))=>s.clone(),_=>return Err(InterpError::runtime("file.rm requires path"))}; let p=std::path::Path::new(&path); let r=if p.is_dir(){std::fs::remove_dir_all(p)}else{std::fs::remove_file(p)}; r.map(|_|Value::Unit).map_err(|e|InterpError::runtime(e.to_string())) })), closure: Env::default() }));
        self.define("file.ls",     Value::Fn(FnValue { name: Some("file.ls".into()),     params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { let dir=match args.first(){Some(Value::Str(s))=>s.clone(),_=>return Err(InterpError::runtime("file.ls requires path"))}; let entries=std::fs::read_dir(&dir).map_err(|e|InterpError::runtime(e.to_string()))?; Ok(Value::List(entries.filter_map(|e|e.ok()).map(|e|Value::Str(e.file_name().to_string_lossy().to_string())).collect())) })), closure: Env::default() }));
        self.define("file.mkdir",  Value::Fn(FnValue { name: Some("file.mkdir".into()),  params: vec![], body: FnBody::Native(std::sync::Arc::new(|args| { let path=match args.first(){Some(Value::Str(s))=>s.clone(),_=>return Err(InterpError::runtime("file.mkdir requires path"))}; std::fs::create_dir_all(&path).map(|_|Value::Unit).map_err(|e|InterpError::runtime(e.to_string())) })), closure: Env::default() }));

        // -- json.* ----------------------------------------------------------
        self.define("json.str", Value::Fn(FnValue {
            name: Some("json.str".into()), params: vec![],
            body: FnBody::Native(std::sync::Arc::new(|args| {
...
impl InterpError {
    pub fn runtime(msg: impl Into<String>) -> Self {
        InterpError { kind: ErrorKind::Runtime, msg: msg.into() }
    }
    pub fn panic(msg: impl Into<String>) -> Self {
        InterpError { kind: ErrorKind::Panic, msg: msg.into() }
    }
    fn return_val(msg: impl Into<String>) -> Self {
        InterpError { kind: ErrorKind::Return, msg: msg.into() }
    }
}
...
impl Interpreter {
    pub fn new() -> Self {
        Interpreter { env: Env::new(), return_value: None }
    }
    ...
    fn exec_stmt(&mut self, stmt: &Stmt) -> InterpResult<Value> {
        match &stmt.kind {
            ...
            StmtKind::Return(expr) => {
                let v = match expr { Some(e) => self.eval_expr(e)?, None => Value::Unit };
                self.return_value = Some(v.clone());
                Err(InterpError { kind: ErrorKind::Return, msg: String::new() })
            }
            StmtKind::Panic(expr) => {
            ...
            StmtKind::For { var, iter, body } => {
                ...
                for item in items {
                    ...
                    match self.exec_block(body) {
                        Ok(_) => {}
                        Err(e) if e.kind == ErrorKind::Return => { self.env.pop(); return Err(e); }
        ...
        match &target.kind {
            ...
            ExprKind::Index { obj, index } => {
                if let ExprKind::Ident(list_name) = &obj.kind {
                    ...
                    let list = self.env.get(list_name)
                        .ok_or_else(|| InterpError::runtime(format!("undefined variable '{list_name}'")))?;
                    match list {
                    ...
                    Err(InterpError::runtime("index assignment only supported on named lists"))
                }
            }
            ...
            ExprKind::Field { obj, field } => {
                if let ExprKind::Ident(obj_name) = &obj.kind {
                    let obj_val = self.env.get(obj_name)
                        .ok_or_else(|| InterpError::runtime(format!("undefined variable '{obj_name}'")))?;
                    match obj_val {
                        ...
                        _ => Err(InterpError::runtime(format!("field assignment on non-struct '{obj_name}'"))),
                    }
                } else {
                    Err(InterpError::runtime("field assignment only supported on named variables"))
                }
            }
    ...
    fn eval_expr(&mut self, expr: &Expr) -> InterpResult<Value> {
        match &expr.kind {
            ...
            ExprKind::Call { callee, args } => {
                ...
                let arg_vals: InterpResult<Vec<_>> = args.iter().map(|a| self.eval_expr(a)).collect();
            ...
            ExprKind::SafeField { obj, field } => {
                ...
                match v {
                    ...
                    Value::Option(Some(inner)) => Ok(Value::Option(Some(Box::new(self.eval_field(*inner, field)?)))),
            ...
            ExprKind::Index { obj, index } => {
                ...
                match (v, i) {
                    (Value::List(items), Value::Int(idx)) => {
                        let idx = if idx < 0 { items.len() as i64 + idx } else { idx } as usize;
                        items.get(idx).cloned().map(|v| Value::Option(Some(Box::new(v))))
                            .ok_or_else(|| InterpError::runtime(format!("index {idx} out of bounds")))
                    }
                    (Value::Map(pairs), key) => {
                        Ok(pairs.into_iter().find(|(k, _)| *k == key)
            ...
            ExprKind::Match { scrutinee, arms } => {
                ...
                for arm in arms {
                    if let Some(bindings) = self.match_pattern(&arm.pattern, &val) {
                        self.env.push();
                        for (name, v) in bindings { self.env.define(&name, v); }
    ...
    fn eval_binop(&mut self, op: &BinOp, lhs: &Expr, rhs: &Expr) -> InterpResult<Value> {
        ...
        match op {
            ...
            BinOp::Div => match (&l, &r) {
                (Value::Int(a),   Value::Int(b))   => {
                    if *b == 0 { return Err(InterpError::runtime("division by zero")); }
                    Ok(Value::Int(a / b))
                }
                (Value::Float(a), Value::Float(b)) => Ok(Value::Float(a / b)),
                (Value::Int(a),   Value::Float(b)) => Ok(Value::Float(*a as f64 / b)),
            ...
            BinOp::Mod => match (&l, &r) {
                (Value::Int(a), Value::Int(b)) => {
                    if *b == 0 { return Err(InterpError::runtime("modulo by zero")); }
                    Ok(Value::Int(a % b))
                }
            ...
            BinOp::Lt    => self.cmp_values(&l, &r).map(|o| Value::Bool(o == std::cmp::Ordering::Less)),
            BinOp::Gt    => self.cmp_values(&l, &r).map(|o| Value::Bool(o == std::cmp::Ordering::Greater)),
            BinOp::LtEq  => self.cmp_values(&l, &r).map(|o| Value::Bool(o != std::cmp::Ordering::Greater)),
            BinOp::GtEq  => self.cmp_values(&l, &r).map(|o| Value::Bool(o != std::cmp::Ordering::Less)),
    ...
    fn eval_field(&mut self, val: Value, field: &str) -> InterpResult<Value> {
        match &val {
            ...
            Value::List(items) => match field {
                ...
                "reverse" => { let mut v = items.clone(); v.reverse(); Ok(Value::List(v)) }
                _ => self.make_method(val.clone(), field),
            },
    ...
    fn make_method(&self, val: Value, method: &str) -> InterpResult<Value> {
        ...
        Ok(Value::Fn(FnValue {
            ...
            body: FnBody::Native(std::sync::Arc::new(move |_args| {
                Err(InterpError::runtime(format!("method '{method}' on {val_clone} not yet implemented")))
            })),
            closure: Env::default(),
        }))
    }
    ...
    fn call_method(&mut self, receiver: Value, method: &str, args: &mut Vec<Value>) -> InterpResult<Value> {
        match (&receiver, method) {
            ...
            (Value::Str(s), "replace") => {
                if args.len() < 2 { return Err(InterpError::runtime("replace() requires 2 args")); }
            ...
            (Value::List(items), "first")   => Ok(items.first().cloned()
                .map(|v| Value::Option(Some(Box::new(v)))).unwrap_or(Value::Option(None))),
            (Value::List(items), "last")    => Ok(items.last().cloned()
                .map(|v| Value::Option(Some(Box::new(v)))).unwrap_or(Value::Option(None))),
            (Value::List(items), "reverse") => {
                let mut v = items.clone(); v.reverse(); Ok(Value::List(v))
            }
            (Value::List(items), "contains") => {
                let target = args.first().ok_or_else(|| InterpError::runtime("contains() requires an arg"))?;
                Ok(Value::Bool(items.contains(target)))
            }
            (Value::List(_), "push") => {
                let val = args.first().cloned().ok_or_else(|| InterpError::runtime("push() requires an arg"))?;
            ...
            (Value::List(items), "sort") => {
                ...
                if args.is_empty() {
                    ...
                    let mut pairs: Vec<(Value, Value)> = v.into_iter()
                        .map(|item| {
                    ...
                    Ok(Value::List(pairs.into_iter().map(|(item, _)| item).collect()))
                }
            }
            (Value::List(items), "filter") => {
                let f = args.first().cloned().ok_or_else(|| InterpError::runtime("filter() requires a fn arg"))?;
                let items = items.clone();
                let mut result = vec![];
                for item in items {
                    let keep = self.call_fn(f.clone(), vec![item.clone()])?;
                    if self.is_truthy(&keep) { result.push(item); }
                }
                Ok(Value::List(result))
            }
            (Value::List(items), "map") => {
                let f = args.first().cloned().ok_or_else(|| InterpError::runtime("map() requires a fn arg"))?;
                let items = items.clone();
                let mut result = vec![];
                for item in items {
            ...
            (Value::List(items), "reduce") => {
                if args.len() < 2 { return Err(InterpError::runtime("reduce() requires (fn, init)")); }
                let f = args[0].clone();
            ...
            (Value::Map(pairs), "get") => {
                ...
                Ok(pairs.iter().find(|(k, _)| k == key)
                    .map(|(_, v)| Value::Option(Some(Box::new(v.clone()))))
                    .unwrap_or(Value::Option(None)))
            }
            (Value::Map(pairs), "has") => {
                let key = args.first().ok_or_else(|| InterpError::runtime("has() requires a key"))?;
            ...
            _ => {
                let fn_val = self.env.get(method)
                    .ok_or_else(|| InterpError::runtime(format!(
                        "no method '{}' on {}", method, receiver
                    )))?;
        ...
        match func {
            Value::Fn(fv) => {
                ...
                FnBody::Ast(stmts) => {
                    ...
                    let saved_locals = std::mem::replace(&mut self.env.locals, fv.closure.locals.clone());
                    self.env.push();
    ...
    fn builtin_filter(&mut self, args: Vec<Value>) -> InterpResult<Value> {
        if args.len() != 2 { return Err(InterpError::runtime("filter() requires (list, fn)")); }
        let items = match &args[0] { Value::List(l) => l.clone(), _ => return Err(InterpError::runtime("filter() first arg must be a list")) };
        let f = args[1].clone();
        let mut result = vec![];
        for item in items {
            let keep = self.call_fn(f.clone(), vec![item.clone()])?;
    ...
    fn builtin_map(&mut self, args: Vec<Value>) -> InterpResult<Value> {
        if args.len() != 2 { return Err(InterpError::runtime("map() requires (list, fn)")); }
        let items = match &args[0] { Value::List(l) => l.clone(), _ => return Err(InterpError::runtime("map() first arg must be a list")) };
        let f = args[1].clone();
        let mut result = vec![];
        for item in items {
            result.push(self.call_fn(f.clone(), vec![item])?);
    ...
    fn builtin_reduce(&mut self, args: Vec<Value>) -> InterpResult<Value> {
        if args.len() < 3 { return Err(InterpError::runtime("reduce() requires (list, fn, init)")); }
        let items = match &args[0] { Value::List(l) => l.clone(), _ => return Err(InterpError::runtime("reduce() first arg must be a list")) };
        let f = args[1].clone();
        let mut acc = args[2].clone();
        for item in items { acc = self.call_fn(f.clone(), vec![acc, item])?; }
        Ok(acc)
    }

    fn builtin_any(&mut self, args: Vec<Value>) -> InterpResult<Value> {
        if args.len() != 2 { return Err(InterpError::runtime("any() requires (list, fn)")); }
        let items = match &args[0] { Value::List(l) => l.clone(), _ => return Err(InterpError::runtime("any() first arg must be a list")) };
        let f = args[1].clone();
        for item in items {
    ...
    fn builtin_all(&mut self, args: Vec<Value>) -> InterpResult<Value> {
        if args.len() != 2 { return Err(InterpError::runtime("all() requires (list, fn)")); }
        let items = match &args[0] { Value::List(l) => l.clone(), _ => return Err(InterpError::runtime("all() first arg must be a list")) };
        let f = args[1].clone();
        for item in items {
            let result = self.call_fn(f.clone(), vec![item])?;
        ...
        match pattern {
            ...
            Pattern::Variant(name, inner_pats) => {
                match val {
                    Value::Variant(vname, fields) if vname == name => {
                        if inner_pats.len() != fields.len() { return None; }
                    ...
                    Value::Ok(v) if name == "Ok" => {
                        if inner_pats.len() == 1 { self.match_pattern(&inner_pats[0], v) } else { None }
                    }
            ...
            Pattern::Tuple(pats) => {
                if let Value::Tuple(fields) = val {
                    if pats.len() != fields.len() { return None; }
                    let mut bindings = vec![];
        ...
        while let Some(c) = chars.next() {
            if c == '{' {
                ...
                if expr_src.chars().all(|c| c.is_alphanumeric() || c == '_') {
                    ...
                    let tokens = ash_lexer::Lexer::new(&expr_src).tokenize()
                        .map_err(|e| InterpError::runtime(format!("interpolation lex error: {e}")))?;
                    let program = ash_parser::parse(tokens)
                        .map_err(|e| InterpError::runtime(format!("interpolation parse error: {}", e.msg)))?;
                    ...
                    if let Some(stmt) = program.stmts.first() {
                        if let ash_parser::ast::StmtKind::Expr(expr) = &stmt.kind {
                            ...
                            return Err(InterpError::runtime(
                                format!("interpolation '{expr_src}' is not an expression")
                            ));
                        }
                    }
                }
...
mod tests {
    ...
    fn test_map_builtin() {
        ...
        assert_eq!(eval(src), Value::List(vec![Value::Int(2), Value::Int(4), Value::Int(6)]));
    }
